...
features[:, 0] = labelencoder.fit_transform(features[:, 0])    ## for first column
...
# Creation of Object
onehotencoder = OneHotEncoder(categorical_features = [0])  ## making a list for the column which have object data 
...
# Prediction for a new values for a person in 
# 'Development', hours worked 1150,
# 3 certificates , 4yrs of experience. 
...
"""
HandsOn
#Import the local file cars.csv and split the data set equally(50%) into test set and training set.
...
# Importing the dataset
dataset = pd.read_csv('All CSV/cars.csv')
dataset.info()
...
train_df.to_csv("All CSV/cars_train.csv")
test_df.to_csv("All CSV/cars_test.csv")
