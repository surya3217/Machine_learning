...
Make a decision Tree based on the data.
...
1.ID3  ----> Entropy based and your calculate Information Gain
  These are statistics keywords
  Entropy is the measurement of Randomness 

Today we will use CART algo for making the decision tree
2.CART  ----> Gini Index/Score
...
If by looking at the feature, if I am NOT ABLE to take decision, 
then it is IMPURE, Ginni index will be MORE

If by looking at the feature, if I am ABLE to take decision, 
then it is PURE, Gini index will be LESS 

How to calculate the Gini Index ?
Gini index would be calculated based on the unique values in it

Now with the above knowlege, we would iterate through the data and create the Tree.
...
used. The final images have 400x400 pixels. Due to the object lens and distance to the 
investigated object gray-scale pictures with a resolution of about 660 dpi were gained. 
Wavelet Transform tool were used to extract features from images.
"""

# This is a Classification problem
dataset = pd.read_csv("All CSV/bill_authentication.csv")  
dataset.info()
...
"""
Problem Definition
The problem here is to predict the petrol consumption (in millions of gallons) 
...
dataset = pd.read_csv('All CSV/petrol_consumption.csv')  
dataset.shape
...
dataset = pd.read_csv('All CSV/Position_Salaries.csv')
...
1.Predict employment of a currently employed 10-year veteran, previous 
  ...
  Internship.
2.Predict employment of an unemployed 10-year veteran, ,previous employers 
  4, didn't went to any top-tire school, having Master's Degree with 
  Internship.
...
which is less than 10 percent of the mean of all the values in the 'Petrol_Consumption'
