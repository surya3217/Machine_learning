...
5.The final value can be calculated by taking the average of all the values predicted by all the trees in forest. 
...
2.This algorithm is very stable. Even if a new data point is introduced 
...
1.A major disadvantage of random forests lies in their complexity. 
2.They required much more computational resources, owing to the large number of decision trees joined together.
...
#parameters of the RandomForestClassifier class and see if you can improve on our 
#results.
...
## Root mean square error < 10 % of mean value for desired column
"""
...
# Importing the dataset
dataset = pd.read_csv('All CSV/sd.csv')
features = dataset.iloc[:, 1:2].values
labels = dataset.iloc[:, 2].values
