# K-Means Clustering
"""
Now the focus has changed to UnSupervised Machine Learning, That means you are 
training your model with features only. It might happen that your dataset does 
have label in it, but you are not using it.

Also it might happen that you dont want to predict something but group your data 
into groups and these groups are known as clusters.
...
dataset = pd.read_csv('All CSV/xclara.csv')
...
1.Display data in 2D Plane, Add Centroid - Randomly places a point as a centroid
  Since we need to have a centroid, but this is placed randomly 
  which is not the right position.
2.Add another Centroid, which now divides the data points into 2 parts or divisions
  It uses a logic wherein it draws a straight line between the two centroid 
  and bisects centrally at 90 degree.
3.Add Centroid again, this will divide into 3 parts, but all the 3 centroid 
  are not at the right place, since they are placed randomly

4.Click GO to fix the data points and starting points of the centroid.
5.Now Click Update Centroids again and again, so that it would change the boundary 
  division, Since it would recalculate the centroid. Now because of recalulation, 
  some of the points will cross boundary of the older divisions.
6.Now Reassign Points based on new division, this should change the centroid again
...
that is the actual centroid. That would be the final clusters.
...
Outer loop for the number of centroid and the inner loop for every point of that 
cluster.
...
#So if you call the silhouette score method for just one cluster, it gives error.
"""
...
If you have pre-existing class labels that you’re trying to duplicate with 
k-means clustering, you can use two measures: homogeneity and completeness.
